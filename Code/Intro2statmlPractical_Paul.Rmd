...
It creates a new column to flag the lines that lie under the alpha with a cumulative sum i.e. summing up from smallest to largest probabilities, flag the k's that have the cumsum under alpha.
...
8) Is there evidence that our coin is biased? Yes
...
No longer sufficient evidence to reject the null hypothesis.
...
```
Look at the 95% CI - it doesn't overlap 0.5: reject null. Now make the test more stringent: 99% CI
...
```
and we now see that the there is no longer sufficient evidence at 99% confidence level. 
...
`sample carries` out the shuffling, replicated 10000 times. 

Question

15) Why did we use the absolute value function (abs) in the above code?
We only care if it falls on one side 
...
SE_b1 = sqrt(var_epsilon/sum((xi - x_mean)^2)) # looks good
